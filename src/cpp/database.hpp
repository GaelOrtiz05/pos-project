...
private:
  ...
  void setupDatabase() {
    ...
    // Table to store items                                        // ID  name
    // price  ingredients
    db.exec(
        "CREATE TABLE IF NOT EXISTS items (" // 01  Cheeseburger         2.99
                                             // "bun,beef,cheese,lettuce,tomato,bun"
        "id               INTEGER PRIMARY KEY AUTOINCREMENT," // 02  Double
                                                              // Cheeseburger  4.99
                                                              // "bun,beef,beef,cheese,lettuce,tomato,bun"
        "name             TEXT NOT NULL UNIQUE," // 03  Chicken
                                                 // Nuggets      3.99 "chicken"
...
public:
  ...
  //   int insertIngredient(const std::string name, double price, int stock =
  //   100) {
...
void setIngredientStock(bool increase, int id, double val = 1) {
    ...
    if (increase) {
      SQLite::Statement query(
          db, "UPDATE ingredients SET stock = stock + ? WHERE id = ?");
  ...
  void addCheckout(int item) {
    SQLite::Statement query(
        db, "INSERT INTO checkout(item,price,ingredient_id) SELECT "
            "name,price,ingredient_id FROM items WHERE id = ?");
    query.bind(1, item);
  ...
  std::string getCheckoutName(int row) {
    ...
    SQLite::Statement query(db, "SELECT item FROM checkout LIMIT 1 OFFSET ?");
    query.bind(1, row);
  ...
  double getCheckoutPrice(int row) {
    SQLite::Statement query(db, "SELECT price FROM checkout LIMIT 1 OFFSET ?");
    query.bind(1, row);
  ...
  void purchase() {
    ...
    try {
      SQLite::Statement query(db, "SELECT ingredient_id FROM checkout");
