...
void Database::Insert_Into_Ingredient_Table(const std::string &name,
...
void Database::Insert_Into_Item_Table(const std::string &name,
                                      const std::string &image, double price,
                                      int categoryId) {
  SQLite::Statement insert(
      db,
...
void Database::Combine_Into_IngredientItem_Table(int ingredientId, int itemId,
                                                 int isRemovable,
                                                 double priceChange) {
  SQLite::Statement insert(
      db, "INSERT INTO item_ingredients (item_id, ingredient_id, is_removable, "
          "price_change) VALUES (?,?,?,?)");
...
bool Database::Check_Ingredient_Stock_Of_Item(int itemID) {
  std::vector<ItemIngredient> vector_of_ingredients =
      Get_Vector_ItemIngredients_by_ItemID(itemID);
...
void Database::Inc_Dec_Ingredient_Stock(bool increase_isTRUE,
                                        const std::string &name, double value) {
  if (increase_isTRUE) {
    SQLite::Statement increase(
        db, "UPDATE ingredients SET stock = stock + ? WHERE name = ?");
    increase.bind(1, value);
    increase.bind(2, name);
    increase.exec();
  } else {
    SQLite::Statement decrease(
        db, "UPDATE ingredients SET stock = stock - ? WHERE name = ?");
...
void Database::Process_Purchase(const std::vector<OrderItem> &Vector_Of_Items,
                                double total) {
  SQLite::Transaction tx(db);

  SQLite::Statement insertOrderId(
      db, R"SQL(INSERT INTO orders (total) VALUES (?) )SQL");
  // insert total
